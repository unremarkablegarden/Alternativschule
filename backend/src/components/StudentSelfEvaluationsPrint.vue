<template lang="pug">
  #selfEvaluations
    ...
    .evals(v-if='selfEvaluations')
      div(:loading='loading', stretch)
        div(v-for='(level, index) in subjectLevels', :key='level', :label="level", :class='level')
          template(slot='label')
            | &nbsp;&nbsp;&nbsp;&nbsp;
            | {{ level }}
            | &nbsp;&nbsp;&nbsp;&nbsp;
          ul
            li.eval(v-for='e in competences', v-if='competencesLoaded')
              strong {{ e.name }}
...
export default {
  name: 'studentselfevaluationsPrint',
